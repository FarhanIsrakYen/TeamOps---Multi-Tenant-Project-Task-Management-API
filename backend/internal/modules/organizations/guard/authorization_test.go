...
func TestPermissionMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{role: orgmodel.RoleAdmin, allowed: map[Permission]bool{ReadOrganization: true, ManageMembers: true, ManageProjects: true, CreateUpdateTasks: true, DeleteTasks: true, CommentTasks: true, ManageLabels: true, ViewAuditLogs: true}},
...
func TestAuthorizationAlwaysReadsCurrentMembership(t *testing.T) {
	t.Parallel()
	...
	reader.mu.Lock()
	reader.role = orgmodel.RoleViewer
	reader.mu.Unlock()
	_, err = guard.RequireOrganizationPermission(ctx, userID, organizationID, ManageMembers)
	require.ErrorIs(t, err, apperror.ErrForbidden)
