...
func (r *Repository) List(ctx context.Context, orgID uuid.UUID, p pagination.Params, filters model.Filters) ([]model.Project, int64, error) {
	sort := map[string]string{"name": "name", "createdAt": "created_at", "updatedAt": "updated_at"}[p.SortBy]
	where := "organization_id=$1"
	args := []any{orgID, p.PageSize, p.Offset}
	if filters.Archived != nil {
		where += " AND archived=$4"
	...
	q := fmt.Sprintf(`SELECT id,organization_id,name,description,archived,version,created_at,updated_at,count(*) OVER() FROM projects WHERE %s ORDER BY %s %s,id %s LIMIT $2 OFFSET $3`, where, sort, order, order)
